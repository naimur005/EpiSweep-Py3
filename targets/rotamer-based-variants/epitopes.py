...
#design_of_interest = filter(lambda x: x[0] == args.index, all_designs)[0]
design_of_interest = list(filter(lambda x: x[0] == args.index, all_designs))[0] #updates

seq = design_of_interest[-1].strip("\n")

epitope_prediction = design.profiles.full_report(seq, design.episcore_threshold)

# Open the file in text mode for CSV writing
with open(args.output, "w", newline='') as file_out: #updates
    output = csv.writer(file_out)
    header = ["Start", "Peptide", "End"] + design.profiles.allele_list + ["Total"]
    output.writerow(header)  # Writes the header to the CSV file
    print(",".join(header))  # Print the header to the console
    ...
    for p, pred in enumerate(epitope_prediction):
        ...
        print(",".join(map(str, line)))  # Print each line to the console
