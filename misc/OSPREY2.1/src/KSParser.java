...
/////////////////////////////////////////////////////////////////////////
	...
	public void handleComputeAllPairwiseRotamerEnergiesMaster(ParamSet sParams) {	
		...
		boolean minimizeBB = (new Boolean((String)sParams.getValue("MINIMIZEBB", "false"))).booleanValue();
		boolean doBackrubs = (new Boolean((String)sParams.getValue("DOBACKRUBS", "false"))).booleanValue();
        boolean ematOnly = (new Boolean((String)sParams.getValue("EMATONLY","false"))).booleanValue();
		String backrubFile ="";
		if(doBackrubs){
			backrubFile = (String)sParams.getValue("BACKRUBFILE");
		}
		...
		*/
		// CBK+ {
		boolean energiesOnlyContacts = (new Boolean((String)sParams.getValue("ENERGIESONLYCONTACTS","false"))).booleanValue();
		float distCutoff = 0;
                if (energiesOnlyContacts) {
		    distCutoff = (new Float((String)sParams.getValue("DISTCUTOFF"))).floatValue();
		    System.out.println("computing pairwise energies only for residues within "+distCutoff);
		}
		OneMutation mutArray[] = getMutArrayPairEcomp(numberMutable,minimizeBB,mp,energiesOnlyContacts,distCutoff);
		// } CBK+
		...
		try{
		    // CBK: replaced MPI with single processor
		    handleDoMPIMaster(mutMan,mutArray.length);
		    //handleSingleProc(mutMan, mutArray.length);
		}
		catch (Exception e){
			System.out.println("ERROR: "+e);
			e.printStackTrace();
		...
		System.out.println("DONE: Pairwise energy matrix precomputation..");
        if (ematOnly) {
            System.out.println("EMATONLY is TRUE. Programme is halted.");
            System.exit(1);
        }
	}
	...
	// CBK mod private OneMutation[] getMutArrayPairEcomp(int numberMutable, boolean minimizeBB){
    private OneMutation[] getMutArrayPairEcomp(int numberMutable, boolean minimizeBB, MolParameters mp, boolean energiesOnlyContacts, float distCutoff){
		...
		if(numMutations >= 2){
			for(int i=0; i<numComb; i++) {
			    ...
			    */
			    ...
			    if (energiesOnlyContacts) {
				// Gross way to find residue pair, but least perturbation to their data structure
				int i1=-1, i2=-1;
				for(int j=0; j<numberMutable; j++) { 
				...
				// following genInteractionGraph
				int str1 = mp.mutRes2Strand[i1];
				int strResNum1 = mp.strandMut[str1][mp.mutRes2StrandMutIndex[i1]];
				int str2 = mp.mutRes2Strand[i2];
				int strResNum2 = mp.strandMut[str2][mp.mutRes2StrandMutIndex[i2]];
				Residue r1 = mp.m.strand[str1].residue[strResNum1];
				Residue r2 = mp.m.strand[str2].residue[strResNum2];
				// following Residue.getDist
				Atom a1=null, a2=null;
				String n1="CB", n2="CB";
				if (r1.name.equals("GLY")) n1="CA";
				if (r2.name.equals("GLY")) n2="CA";
				for (Atom a : r1.atom) if (a.name.equals(n1)) a1 = a;
				for (Atom a : r2.atom) if (a.name.equals(n2)) a2 = a;
				float d1 = a1.coord[0] - a2.coord[0], d2 = a1.coord[1] - a2.coord[1], d3 = a1.coord[2] - a2.coord[2];
				float sqd = d1*d1 + d2*d2 + d3*d3;
				// System.out.println(r1.fullName+" - "+r2.fullName+" = "+Math.sqrt(sqd));

				goodPair = sqd < distCutoff*distCutoff;
			    }

			    // now add mutation only if in contact distance
			    if (goodPair) {
				mutArray[curMutNum] = new OneMutation();
				mutArray[curMutNum].flagMutType = "AS-AS";
				mutArray[curMutNum].resMut = new int[numberMutable];
				for(int j=0; j<numberMutable; j++) {
					mutArray[curMutNum].resMut[j] = residueMutatable[i][j];
				}
...
///////////////////////////////////////////////////////////////////////////
	...
	*/
	public void handleDoDEE(String s) {
		...
		// CBK+
		boolean pruneOnly = (new Boolean((String)sParams.getValue("PRUNEONLY","false"))).booleanValue();
		String rotamerChoiceName = (String)(sParams.getValue("ROTAMERCHOICENAME",runName+"_rot_choices.csv" )); // CBK+ YJ+
		...
		do{
            ...
            // CBK+ {  write remaining rotamers every cycle
                String[] AAOneLetterCode = { "A", "V", "L", "I", "F",
                    "Y", "W", "C", "M", "S", "T", "K", "R", "H", "H",
                    "H", "D", "E", "N", "Q", "G", "P" };
				if (pruneOnly) {
				    PrintStream out = System.out;
				    try {
					    out = new PrintStream(new BufferedOutputStream(new FileOutputStream(rotamerChoiceName)));
				    }
				    catch (IOException e) {
					    System.out.println("couldn't open rotamer output file");
				    }
				    // out.println("pos,aa,rot");
				    // Based on PairwiseEnergyMatrix.reduceMatrix
				    int treeLevels = mp.numberMutable;
				    for (int curLevel=0; curLevel<treeLevels; curLevel++){
					int str = rs.mutRes2Strand[curLevel];
					int strResNum = mp.strandMut[str][rs.mutRes2StrandMutIndex[curLevel]];
					int molResNum = rs.m.strand[str].residue[strResNum].moleculeResidueNumber;
					int numAA=rs.strandRot[str].getNumAllowable(strResNum);
					for (int curAA=0; curAA<numAA; curAA++){ //for all allowed AA's
						...
						if (!rs.eliminatedRotAtRes.get(curLevel,index,curRot)){ //not pruned, so add its index
						    out.println((curLevel+1)+","+AAOneLetterCode[index]+","+curRot);
						    System.out.println((curLevel+1)+","+AAOneLetterCode[index]+","+curRot);
						}
					    }
					}
				    }
				...
				// } CBK+
                // YJ comment: It was at the beginning of the function but moved to the end
                // as this fits the reduced energy matrix after pruning rotamers.
                // Another changes: 
...
///////////////////////////////////////////////////////////////////////////
    ...
    // CBK+ this function replaces handleDoMPIMaster; modeled on it
    // Never fully tested for correctness -- once identified core bottleneck, went back to multithread
    public void handleSingleProc(MutationManager mutMan, int size) {
	MPItoThread.threadEle.get(Thread.currentThread()).rank = -1;
	for (int curMut = 0; curMut < size; curMut++) {
	    System.out.println("Retrieving "+curMut+" of "+size);
	    CommucObj curObj = mutMan.getNextComObj(curMut);
	    curObj = handleKSSlave(curObj);
	    mutMan.processFinishedMutation(curObj);
	}
    }
