...
//// BEGIN CHECK_STERICS CODE SECTION
    ...
    // CBK: version for side-chain against backbone
    private boolean stericsSCvsBB(int strandNum, int resNum) {
	ProbeStericCheck psc = new ProbeStericCheck();
	...
	for(int i=0;i<res.numberOfAtoms;i++) {
		...
		for(int q=0;q<m.numberOfStrands;q++) {
			...
			if(!((q==strandNum) && (w==resNum))) {
			    for(int t=0;t<m.strand[q].residue[w].numberOfAtoms;t++) {
				Atom a2 = m.strand[q].residue[w].atom[t];
				if ( ( a2.getIsBBatom() && (!doPerturbations) ) ) {
				    //For methods other than DEEPer, check only the backbone atoms for the sysStrand residues that are in excludeList[]
				    //for DEEPer, these backbone atoms may move a lot, so exclude residues in excludeList entirely
				    if ( hSteric || (!a2.elementType.equalsIgnoreCase("H"))) {
					if (!psc.isAllowedSteric(m, a1, a2, softOverlapThresh))
    ...
    // CBK: version for one side-chain against another
    private boolean stericsSCvsSC(int strandNum1, int resNum1, int strandNum2, int resNum2) {
	ProbeStericCheck psc = new ProbeStericCheck();
	
	Residue res1 = m.strand[strandNum1].residue[resNum1];
	Residue res2 = m.strand[strandNum2].residue[resNum2];
		
	for(int i=0;i<res1.numberOfAtoms;i++) {
	    Atom a1 = res1.atom[i];
	    if ( hSteric || (!a1.elementType.equalsIgnoreCase("H")) ) {
		for(int t=0;t<res2.numberOfAtoms;t++) {
		    Atom a2 = res2.atom[t];
		    if ( hSteric || (!a2.elementType.equalsIgnoreCase("H"))) {
			if (!psc.isAllowedSteric(m, a1, a2, softOverlapThresh))
			    return false;
...
//validConf indicates that all RC applications were successful (always true if not DEEPer)
	    ...
	    if (curMut>=numMut) {
				...
				/* CBK-  -- separate cases for sterics, so don't keep recomputing single in pair
				...
				*/
				...
				boolean stericallyGood = false;
				if (res2 == -1) {
				    // steric energy calculation for shell: against all BB atoms
				    stericallyGood = stericsSCvsBB(res1Strand,res1Resnum);
				}
				else {
				    // steric energy calculation for pair: against the other residue
				    // (assuming that they'd be eliminated for steric clash on their own so don't need to re-handle)
				    stericallyGood = stericsSCvsSC(res1Strand, res1Resnum, res2Strand, res2Resnum);
				}
				// } CBK+
